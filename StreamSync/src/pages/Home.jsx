...
const Home = () => {
  ...
  return (
    <div className='bg-purple-50/50 dark:bg-zinc-950 min-h-screen'>
      ...
      <nav className="flex p-4 sm:p-6 px-4 sm:px-8 md:px-12 lg:px-20 xl:px-40 justify-between items-center transition-colors duration-300">
        <div>
        ...
        <div className='flex items-center gap-2 sm:gap-4'>
          <button
            onClick={toggleTheme}
            className="flex items-center gap-2 px-2 py-2 rounded-full bg-neutral-100 dark:bg-neutral-900 text-sm text-black dark:text-white hover:scale-105 transition-all duration-300 shadow-sm"
      ...
      <div className="text-center space-y-2 sm:space-y-4 mt-8 sm:mt-12 md:mt-[6rem] px-4">
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-zinc-800 dark:text-white">
          Watch Together.
        </h1>
        <h1 className="bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent text-3xl sm:text-4xl md:text-5xl lg:text-7xl font-bold tracking-tight">
          From Anywhere.
        </h1>
      </div>

      <div className='flex flex-col items-center mt-4 sm:mt-5 gap-1 sm:gap-2 px-4'>
        <h2 className='dark:text-neutral-300 text-lg sm:text-xl md:text-2xl max-w-3xl sm:max-w-4xl text-center text-black'>
          Create or join a room to watch YouTube videos in perfect sync with friends.
        </h2>
        <div className='mt-3 sm:mt-5 flex max-w-full overflow-x-auto sm:overflow-visible pb-2 sm:pb-0'>
          <ul className='flex items-center gap-4 sm:gap-6 md:gap-10 px-2 sm:px-0'>
            <li className='dark:text-neutral-200 text-black flex items-center gap-1 sm:gap-2 text-sm sm:text-base'>
        ...
        <div className="flex flex-col sm:flex-row justify-center mt-8 sm:mt-14 gap-6 sm:gap-8 md:gap-14 px-4 w-full max-w-6xl">

          <div className="flex flex-col items-center bg-white dark:bg-neutral-900 p-5 sm:p-7 rounded-xl sm:rounded-2xl shadow-lg sm:shadow-xl w-full sm:w-[20rem] md:w-[22rem] space-y-3 sm:space-y-4 transition-all duration-300">
            <div className="flex items-center justify-center bg-purple-500 p-4 sm:p-5 rounded-full shadow hover:scale-105 transition-transform">
          ...
          <div className="flex flex-col items-center bg-white dark:bg-neutral-900 p-5 sm:p-7 rounded-xl sm:rounded-2xl shadow-lg sm:shadow-xl w-full sm:w-[20rem] md:w-[22rem] space-y-3 sm:space-y-4 transition-all duration-300">
            <div className="flex items-center justify-center bg-blue-400 p-4 sm:p-5 rounded-full shadow hover:scale-105 transition-transform">
      ...
      <div id="about" className="flex justify-center px-4 sm:px-6 pb-16 sm:pb-24 pt-16 sm:pt-28 transition-colors duration-300">
        <div className="flex flex-col items-center w-full max-w-4xl bg-white dark:bg-zinc-900 rounded-xl sm:rounded-2xl shadow-lg sm:shadow-2xl p-6 sm:p-8 md:p-10 space-y-8 sm:space-y-11">
          ...
          <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-8 w-full">
            {[
              ...
              <div key={index} className="flex flex-col items-center text-center space-y-2 sm:space-y-3 p-3 sm:p-4">
                <img src={item.icon} className="w-10 sm:w-12 md:w-14 p-2 sm:p-3 rounded-lg bg-zinc-50 dark:bg-zinc-800" alt="" />
