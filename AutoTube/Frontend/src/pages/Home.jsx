...
const Home = () => {
  ...
  const markAsComplete = async () => {
    ...
    try {
      ...
      const updatedVideo = updatedVideos.find(v => v.videoUrl === latestVideo.videoUrl);
    ...
    <>
      ...
      <div className='bg-[#f4f9ff] min-h-screen font-sans text-gray-800'>
        ...
        >
          ...
          >
            Paste any YouTube URL to get AI-powered summaries, QuickQuiz, and track your learning journey
          </motion.p>
        ...
        {latestVideo.title && (
          ...
          >
            <div className='w-full relative'>
              <img src={latestVideo.thumbnail} className='w-full h-48 sm:h-64 object-cover rounded-xl' alt="" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent rounded-xl"></div>
              <div className='absolute right-0 bottom-0 left-0 text-white p-4'>
                <h1 className='text-sm sm:text-lg font-semibold line-clamp-2'>{latestVideo.title}</h1>
                <div className='flex items-center gap-1 text-xs sm:text-sm text-gray-300'>
                  <img src={usersvg} className='w-3 h-3 sm:w-4 sm:h-4' alt="User" />
              ...
              <div className='flex flex-wrap justify-center sm:justify-start gap-1 sm:gap-2'>
                ...
                >
                  <img 
                    ...
                    alt="" 
                  />
                  <span>{latestVideo.isCompleted ? "Completed" : "Mark Complete"}</span>
                </motion.button>
              </div>
            </div>
            ...
            <div className='p-3 sm:p-5 w-full'>
              ...
              >
                {activeTab === "summary" ? (
                  <div>
                    <h1 className='font-bold text-lg sm:text-xl text-purple-700'>AI Summary</h1>
                    <p className='text-gray-700 text-sm sm:text-base mb-3 sm:mb-4'>{latestVideo.summary}</p>

                    {latestVideo.keyPoints && latestVideo.keyPoints.length > 0 && (
                      <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        transition={{ delay: 0.2 }}
                      >
                        <h2 className='font-semibold text-base sm:text-lg text-purple-600'>Key Points</h2>
                  ...
                  <div>
                    <h1 className='font-bold text-lg sm:text-xl text-purple-700'>QuickQuiz</h1>
                    <ul className='list-decimal pl-4 sm:pl-5 space-y-1 sm:space-y-2 text-sm sm:text-base'>
                      {latestVideo.qna.map((item, index) => (
                        <motion.li 
                          ...
                          transition={{ delay: 0.1 * index }}
                        >
                          <p className='text-blue-700 font-semibold'>{item.question}</p>
                  ...
                  <p className='text-gray-500 italic text-sm sm:text-base'>No questions generated.</p>
                )}
              </motion.div>
            </div>
          </motion.div>
        ...
        >
          <div className='p-2 sm:p-4 space-y-3 sm:space-y-5 w-full lg:w-1/3'>
            ...
            >
              <div className='flex items-center gap-2'>
                <img src={historysvg} className='w-6 h-6 sm:w-8 sm:h-8' alt="" />
                <h1 className='font-bold text-lg sm:text-xl'>Videos Watched</h1>
              </div>
              <div className='text-center'>
                <h1 className='font-bold text-xl sm:text-2xl'>{watchedVideos}</h1>
                <p className='text-xs sm:text-base text-neutral-500'>{completedVideos} Completed</p>
              </div>
            </motion.div>
          </div>
          ...
          >
            <div className='p-3 sm:p-5'>
              ...
              <p className='text-xs sm:text-base text-neutral-500'>Your Latest video summaries</p>
            </div>
            ...
            <div className='space-x-2 overflow-y-auto overflow-x-hidden max-h-96'>
                ...
                >
                  <div className='flex'>
                    ...
                    <div className='space-y-1 sm:space-y-2 relative p-1 sm:p-2'>
                      <img 
                        src={completesvg} 
                        className={`${video.isCompleted ? "" : "hidden"} top-0 right-1 sm:right-2 absolute w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7`} 
                        alt="" 
                      />
                      <h1 className='font-semibold text-xs sm:text-sm md:text-base line-clamp-1 sm:line-clamp-2'>{video.title}</h1>
                      <div className='flex gap-2 sm:gap-4 md:gap-6 text-neutral-500 text-xs sm:text-sm md:text-base'>
                        <h1 className='flex items-center gap-1'>
                          <img src={usersvg} className='w-3 h-3 sm:w-4 sm:h-4' alt="" />
                          <span className='line-clamp-1'>{video.author}</span>
                        </h1>
                        <p>{video.time}</p>
                      </div>
                      <p className='text-xs text-gray-700 line-clamp-1 sm:line-clamp-2'>{video.summary?.slice(0, 3560)}...</p>
              ...
              {user.history && user.history.length === 0 && (
                ...
                >
                  ...
                  <h1 className='font-semibold text-base sm:text-lg md:text-xl text-neutral-500'>No Learning History</h1>
                  <p className='text-xs sm:text-sm md:text-base text-neutral-500'>Start by analyzing your first YouTube video</p>
                </motion.div>
              )}
            </div>
          </motion.div>
